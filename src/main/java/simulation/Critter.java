...
public class Critter {
    ...
    private CritterInterpreter critterInterpreter;
    private volatile int x;
    private volatile int y;
    private volatile String species = "unknown";
    ...
    private volatile String lastRule = "";
     ...
     * @param critterInterpreter behavior provider
     */
    public Critter(int x, int y, int direction, int energy, CritterInterpreter critterInterpreter) {
        this(x, y, direction, energy, DEFAULT_SIZE, critterInterpreter);
    }
     ...
     * @param critterInterpreter behavior provider
     */
    public Critter(int x, int y, int direction, int energy, int size, CritterInterpreter critterInterpreter) {
        this.x = x;
        this.y = y;
        ...
        this.critterInterpreter = Objects.requireNonNull(critterInterpreter, "critterInterpreter");
     ...
     * @param critterInterpreter behavior provider
     */
    public Critter(int energy, int direction, CritterInterpreter critterInterpreter) {
        this(0, 0, direction, energy, critterInterpreter);
    }
     ...
     */
    public CritterInterpreter getCritterInterpreter() {
        return critterInterpreter;
    }

    /**
     * Returns the critter species name.
     */
    ...
    /**
     * Returns the last fired rule string (if any).
     */
    public String getLastRule() {
        return lastRule == null ? "" : lastRule;
    }

    /**
     * Sets the last fired rule string.
     */
    public void setLastRule(String rule) {
        this.lastRule = rule == null ? "" : rule;
    }

    /**
     * Updates the interpreter for this critter (used by mutation system).
     *
     * @param newCritterInterpreter new interpreter to use
     */
    public void updateInterpreter(CritterInterpreter newCritterInterpreter) {
        this.critterInterpreter = Objects.requireNonNull(newCritterInterpreter, "newCritterInterpreter");
    }
    ...
    public void setMemorySize(int newSize) {
        int updatedSize = Math.max(MIN_MEMORY_SIZE, newSize);
        if (this.memorySize != updatedSize) {
            this.memorySize = updatedSize;
            if (this.memory != null) {
                int[] newMemory = new int[this.memorySize];
                System.arraycopy(this.memory, 0, newMemory, 0, Math.min(this.memory.length, this.memorySize));
    ...
    public void writeMemory(int index, int value) {
        ...
        // Special case: posture (memory[6]) ignores updates out of bounds
        if (index == 6) {
            if (value < MIN_POSTURE || value > MAX_POSTURE) {
                return;
            }
            memory[6] = value;
            posture = value;
    ...
    private ProgramCritterInterpreter getInterpreterIfAvailable() {
        if (critterInterpreter instanceof ProgramCritterInterpreter) {
            return (ProgramCritterInterpreter) critterInterpreter;
        }
        return null;
    }
    ...
    public void adjustEnergy(int delta) {
        energy += delta;
        int maxEnergy = Constants.ENERGY_PER_SIZE * size;
        if (energy > maxEnergy) {
            energy = maxEnergy;
        }
        if (energy <= 0) {
    ...
    public Critter createOffspring(int initialEnergy) {
        Critter offspring = new Critter(0, 0, direction, initialEnergy, DEFAULT_SIZE, critterInterpreter.offspringCopy());
